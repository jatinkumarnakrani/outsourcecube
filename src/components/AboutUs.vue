...
<template>
    <section :id="id" class="section-pad bg-hero-glow overflow-x-clip">
        <div class="container-shell grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]">
            <div class="text min-w-0" data-aos="flip-right">
                <Badge :className="['bg-surface','text-brand-deep']">{{about?.badge}}</Badge>
            ...
            <div class="relative min-w-0 min-h-[320px] sm:min-h-[420px] p-8" data-aos="flip-left">
                <img :src="websiteStore.getImagePath(about?.image)" :alt="about?.imageAlt" class="mx-auto w-full max-w-xs sm:max-w-sm lg:max-w-96 h-auto object-contain"/>
                <FloatingCube
                    top="-20px"
                    left="-20px"
                    :size="80"
